package com.ankit.FlatMap;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.stream.Collectors;

public class Demo2 {
public static void main(String[] args) {
	List<String>teamA = Arrays.asList("sachya","raman","pistul");
	List<String>teamB = Arrays.asList("balwant","shantaram","janki");
	List<String>teamC = Arrays.asList("vijayalakshmi","pirti","kanchal");
 
List<List<String>> playersInWorldCup = new ArrayList<>();

playersInWorldCup.add(teamA);
playersInWorldCup.add(teamB);
playersInWorldCup.add(teamC);

// Before java 8
for (List<String> team : playersInWorldCup) {
	for(String name:team) {	
	System.out.println(name);
}
}
System.out.println("**********************");

// using stream flatMap()
List<String>names = playersInWorldCup.stream().flatMap(plist->plist.stream()).collect(Collectors.toList());
System.out.println(names);

}
}
